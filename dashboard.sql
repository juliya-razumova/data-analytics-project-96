...
with ads_visit as (
    select
        sessions.visitor_id,
        max(sessions.visit_date) filter (where sessions.medium = 'organic') as organic_date,
        max(sessions.visit_date) filter (where sessions.medium != 'organic') as ads_date
    from sessions
...
tab as (
    select 
        ...
        case
	        when ads_visit.ads_date is null then cast (ads_visit.organic_date as date)
    ...
    left join leads
    on ads_visit.visitor_id = leads.visitor_id
...
select
    ...
    round(100.0 * count(tab.lead_id) filter (where tab.amount > 0)/ count(tab.lead_id), 2) as purchases_conv
from tab
...
select
    ...
    sum(tab.amount) as revenue,
        round(100.0 * count(tab.lead_id) / count(tab.visitor_id), 2) as lead_conv,
        round(100.0 * count(tab.lead_id) filter (where tab.amount > 0)/ count(tab.lead_id), 2) as purch_conv
from tab
...
on conv.utm_source = ads.utm_source
...
select
    tab.utm_source,
    case 
        when tab.utm_source = 'yandex' then percentile_disc(0.9) within group (order by tab.created_at - visit_date)
        when tab.utm_source = 'vk' then percentile_disc(0.9) within group (order by tab.created_at - visit_date)
    end as percent_90
from tab
where rn = 1
group by tab.utm_source;
...
conv as (
    ...
    from tab
    where rn = 1
    group by cast(tab.visit_date as date), tab.utm_source, tab.utm_campaign
)
